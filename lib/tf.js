...
export function threefish256EncryptBlock(key, block, uint32out) {
  ...
  const ks = [
    ...
    keyView[2],
    keyView[3],
    keyView[0] ^ keyView[1] ^ keyView[2] ^ keyView[3] ^ SKEIN_KS_PARITY,
  ];
  let x0 = (blockView[0] + ks[0]) & MAX_BIG_UINT64,
      x1 = (blockView[1] + ks[1]) & MAX_BIG_UINT64,
      x2 = (blockView[2] + ks[2]) & MAX_BIG_UINT64,
      x3 = (blockView[3] + ks[3]) & MAX_BIG_UINT64;
  ...
  if (uint32out) {
    ...
    return new BigUint64Array([x0, x1, x2, x3]).buffer;
  }
}
