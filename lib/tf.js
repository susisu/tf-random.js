 ...
 * This implementation is based on the reference implementation available at the Skein official
 * website (http://www.skein-hash.info) modified to adapt the interface of tf-random.
...
export function threefish256EncryptBlock(key, block, int32out) {
  ...
  for (let r = 1; r <= SKEIN_256_ROUNDS_TOTAL / 8; r++) {
    $add(x0, x1); $rotateL(x1, R_256_0_0); $xor(x1, x0);
    $add(x2, x3); $rotateL(x3, R_256_0_1); $xor(x3, x2);

    $add(x0, x3); $rotateL(x3, R_256_1_0); $xor(x3, x0);
    $add(x2, x1); $rotateL(x1, R_256_1_1); $xor(x1, x2);

    $add(x0, x1); $rotateL(x1, R_256_2_0); $xor(x1, x0);
    $add(x2, x3); $rotateL(x3, R_256_2_1); $xor(x3, x2);

    $add(x0, x3); $rotateL(x3, R_256_3_0); $xor(x3, x0);
    $add(x2, x1); $rotateL(x1, R_256_3_1); $xor(x1, x2);

    injectKey(2 * r - 1);

    $add(x0, x1); $rotateL(x1, R_256_4_0); $xor(x1, x0);
    $add(x2, x3); $rotateL(x3, R_256_4_1); $xor(x3, x2);

    $add(x0, x3); $rotateL(x3, R_256_5_0); $xor(x3, x0);
    $add(x2, x1); $rotateL(x1, R_256_5_1); $xor(x1, x2);

    $add(x0, x1); $rotateL(x1, R_256_6_0); $xor(x1, x0);
    $add(x2, x3); $rotateL(x3, R_256_6_1); $xor(x3, x2);

    $add(x0, x3); $rotateL(x3, R_256_7_0); $xor(x3, x0);
    $add(x2, x1); $rotateL(x1, R_256_7_1); $xor(x1, x2);
