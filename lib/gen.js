...
function mash(key, count, bits, uint32out) {
  ...
  const crypt = new ArrayBuffer(32);
  threefish256EncryptBlock(key, block, crypt, uint32out);
  return crypt;
}
...
export class TFGen {
  ...
  next() {
    ...
    if (this.count < MAX_UINT64 - 1n) {
      ...
      const newKey = mash(this.key.buffer, MAX_UINT64, this.bits, false);
  ...
  splitn(nbits, i) {
    ...
    if (this.bitsIndex + nbits > 64) {
      const newKey = new BigUint64Array(mash(
        this.key.buffer,
        this.count,
        this.bits | ((b << BigInt(this.bitsIndex)) & MAX_UINT64),
        false
      ));
      return makeTFGen(
        newKey,
