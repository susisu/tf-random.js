...
export class TFGen {
  ...
  split() {
    if (this.bitsIndex === 64) {
      ...
      return [makeTFGen(newKey, ZERO, ZERO, 1), makeTFGen(newKey, ZERO, ONE, 1)];
    } else {
      return [
  ...
  splitn(nbits, i) {
    ...
    if (this.bitsIndex + nbits > 64) {
      ...
      return makeTFGen(newKey, ZERO, shiftR(b, 64 - this.bitsIndex), nbits - (64 - this.bitsIndex));
    } else {
      return makeTFGen(
        this.key,
        this.count,
 ...
 */
function mash(key, count, bits, int32out) {
 ...
 * @returns {(uint64|number)[]} 4 * 64-bit uint, or 8 * 32-bit int if `int32out` is true.
 */
function mashTFGen(gen, int32out) {
  return mash(gen.key, gen.count, gen.bits, int32out);
}
 ...
 * @private
 * @returns {number} A random 32-bit int.
 */
function nativeRandomInt32() {
