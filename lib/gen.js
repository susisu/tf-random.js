...
/**
 * @param {bigint[]} key - Hash key (4 * 64-bit uint)
 ...
 * @param {bigint} count - Counter of `next` operations (64-bit uint).
 * @param {bigint} bits - History of `split` operations (64-bit uint).
 * @param {bigint} bitsIndex - Index that new bit will be added to `bits` at next `split`.
 * @returns {TFGen}
 */
...
export class TFGen {
   ...
   */
  static seed(a, b, c, d) {
    return makeTFGen([a, b, c, d], 0n, 0n, 0n);
  }
  ...
  next() {
    ...
    else if (this.bitsIndex < 64n) {
      nextGen = makeTFGen(this.key, 0n, setBit(this.bits, this.bitsIndex), this.bitsIndex + 1n);
    }
    else {
      const newKey = mash(this.key, MAX_BIG_UINT64, this.bits, false);
      nextGen = makeTFGen(newKey, 0n, 0n, 0n);
    }
    return [val, nextGen];
  }
   ...
   */
  split() {
    if (this.bitsIndex === 64n) {
      const newKey = mashTFGen(this, false);
      return [
        makeTFGen(newKey, 0n, 0n, 1n),
        makeTFGen(newKey, 0n, 1n, 1n),
      ];
    }
    else {
      return [
        makeTFGen(this.key, this.count, this.bits, this.bitsIndex + 1n),
        makeTFGen(this.key, this.count, setBit(this.bits, this.bitsIndex), this.bitsIndex + 1n),
      ];
    }
  }
   ...
   */
  level() {
    if (this.bitsIndex + 40n > 64n) {
      const newKey = mashTFGen(this, false);
      return makeTFGen(newKey, 0n, 0n, 0n);
    }
    else {
      return this;
    }
  ...
  splitn(nbits, i) {
    ...
    if (this.bitsIndex + BigInt(nbits) > 64n) {
      const newKey = mash(
        this.key,
        this.count,
        this.bits | ((b << this.bitsIndex) & MAX_BIG_UINT64),
        false
      );
      return makeTFGen(
        newKey,
        0n,
        b >> (64n - this.bitsIndex),
        BigInt(nbits) - (64n - this.bitsIndex)
      );
    }
    else {
      return makeTFGen(
        this.key,
        this.count,
        this.bits | (b << this.bitsIndex),
        this.bitsIndex + BigInt(nbits)
      );
    }
  }
}
