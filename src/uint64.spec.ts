...
function uint64(hi: number, lo: number): WUint64 {
  return [lo | 0, hi | 0];
}

function dump(x: Uint64): string {
  const lo = (x[0] >>> 0).toString(16).padStart(8, "0");
  const hi = (x[1] >>> 0).toString(16).padStart(8, "0");
  return `0x${hi}_${lo}`;
}

describe("fromUint32", () => {
  it("should casts a 32-bit unsigned integer to `Uint64`", () => {
...
describe("fromSafeInt", () => {
  ...
  it("should throw an error if given number is not a safe integer", () => {
    expect(() => {
      ...
      fromSafeInt(0xffffffffffffffff);
    }).toThrow(Error);
  });
});
